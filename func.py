...
class ModelPerformance:
    ...
    def train_test(
        self, dataset, test_size=0.33, train_set=None, test_set=None, seed=42
    ):
    ...
    def prediction(self, predict_dataset=None, Use_train=True, Use_test=False):
        ...
        if Use_train:
            ...
            if "target" in predict_dataset.columns.values:
                feat_name = list(
                    set(predict_dataset.columns.values).difference({"target"})
                )
...
class EP:
    ...
    def fig_display_2d(
        self, pca, pca_name, model, e_profile, add_dataset, file_name=None, save=True
    ):
        print(self.dataset)
        df_px_1 = pd.concat(
            (
                pca,
                e_profile,
                pd.DataFrame(add_dataset, index=["hover name"]).T,
                pd.DataFrame(
                    np.repeat("att", len(self.dataset)), columns=["data type"]
                ),
            ),
            axis=1,
        )
        df_px_2 = pd.concat(
            (
                ...
                pd.DataFrame(add_dataset, index=["hover name"]).T,
                pd.DataFrame(
                    np.repeat("raw", len(self.dataset)), columns=["data type"]
                ),
            ),
            axis=1,
        )
        ...
        fig = px.scatter(
            ...
            facet_col="data type",
            hover_data=self.dataset.columns.values,
            opacity=0.8,
            title=file_name,
        ...
        for j in tqdm(np.arange(len(self.X))):
            if mode == "classification":
                exp = explainer.explain_instance(
                    self.X[j, :], model.predict_proba, num_features=len(self.feat_name)
                )
            else:
                exp = explainer.explain_instance(
                    self.X[j, :], model.predict, num_features=len(self.feat_name)
                )
            exp_prof_lime = dict()
