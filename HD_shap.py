...
from func import ModelPerformance, ShapExplanation, log
...
profile_shap_pca = pd.DataFrame(
    ep_explanation.dim_reduct(e_profile=profile_shap), columns=["PC1", "PC2"]
)

pca_name = "MDS"
model_name = str(clf).split("(")[0]

ep_explanation.fig_display_2d(
    model=model_name,
    pca=profile_shap_pca,
    e_profile=profile_shap,
...
X_pca = pd.DataFrame(
    ep_explanation.dim_reduct(e_profile=explain_X), columns=["PC1", "PC2"]
)
ep_explanation.fig_display_2d(
    model=model_name,
    pca=X_pca,
